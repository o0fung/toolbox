...
def tree(
    ...
    hidden: bool = typer.Option(False, '-h', '--hidden', help='Skip hidden files and directories')
):
    """
    ...
    Args:
        ...
        hidden (bool): If True, skip hidden files and directories.
    """
    tree_obj = Tree(f"Directory: {path}")       # Create the root of the tree
    add_tree(tree_obj, path, depth, current_level=1, skip_hidden=hidden)
    print(tree_obj)                             # Print the tree using rich


def add_tree(tree, path, max_depth, current_level, skip_hidden=False):
    """
    ...
    try:
        ...
        for entry in os.listdir(path):
            ...
            if os.path.isdir(full_path):            
                ...
                add_tree(branch, full_path, max_depth, current_level + 1, skip_hidden)
